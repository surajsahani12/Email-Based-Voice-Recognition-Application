...
def dashboard():
    import dashboard_duplicate


def register_new():
    root.destroy()
    import register_duplicate
...
class Login:
    ...
    def __init__(self, root):
        ...
        pass_value = Entry(self.root, textvariable=self.entry_pass, font=('times new roman,', 15, 'bold'),
                           bg='lightgray')
        btn_login = Button(self.root, text="Login", fg="white", bg="purple", command=self.login,
                           font=("times new roman", 18, "bold"), cursor="hand2")
        or_lable = Label(self.root, text="Or", font=("times new roman", 15, "bold"))
        btn_regi = Button(self.root, text="Register new Account?", command=register_new, bd=0, font=("times new roman", 15, "bold"),
